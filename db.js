...
const JSON_KNOWLEDGE_PATH = path.join(DATA_DIR, 'knowledge.json');
...
if (!fs.existsSync(JSON_KNOWLEDGE_PATH)) {
    fs.writeFileSync(JSON_KNOWLEDGE_PATH, JSON.stringify([], null, 2), 'utf-8');
}
...
const KnowledgeSchema = new mongoose.Schema({
    ...
    fixAction: { type: String, required: true },
    severity: { type: String, required: true },
    priority: { type: String, required: true },
...
const db = {
    ...
    insertKnowledge: async (kData) => {
        if (!isFallback) {
            try {
                const existing = await KnowledgeModel.findOne({ bugId: kData.bugId });
                if (existing) {
                    Object.assign(existing, kData);
                    return await existing.save();
                }
                const doc = new KnowledgeModel(kData);
                return await doc.save();
            } catch (e) {
                logger.error(`MongoDB insert knowledge error: ${e.message}. Attempting fallback writing.`);
            }
        }
        
        // Local File Fallback
        try {
            const dataStr = fs.readFileSync(JSON_KNOWLEDGE_PATH, 'utf-8');
            const list = JSON.parse(dataStr);
            const idx = list.findIndex(item => item.bugId === kData.bugId);
            const entry = {
                ...kData,
            ...
            fs.writeFileSync(JSON_KNOWLEDGE_PATH, JSON.stringify(list, null, 2), 'utf-8');
            logger.info(`Saved Knowledge ${kData.bugId} inside local JSON fallback database.`);
            return entry;
        } catch (e) {
            logger.error(`Fallback database write failed: ${e.message}`);
            throw e;
    ...
    getKnowledge: async () => {
        if (!isFallback) {
            try {
                return await KnowledgeModel.find().sort({ resolutionDate: -1 });
            } catch (e) {
                logger.error(`MongoDB knowledge list error: ${e.message}. Using fallback reading.`);
            }
        }
        ...
        try {
            const dataStr = fs.readFileSync(JSON_KNOWLEDGE_PATH, 'utf-8');
            const list = JSON.parse(dataStr);
            return list.sort((a, b) => new Date(b.resolutionDate) - new Date(a.resolutionDate));
        } catch (e) {
            logger.error(`Fallback database knowledge list reading failed: ${e.message}`);
            return [];
        }
    },

    updateKnowledge: async (bugId, updates) => {
        if (!isFallback) {
            try {
                return await KnowledgeModel.findOneAndUpdate({ bugId }, { $set: updates }, { new: true });
            } catch (e) {
                logger.error(`MongoDB update error: ${e.message}. Using fallback updating.`);
            }
        }
        ...
        try {
            const dataStr = fs.readFileSync(JSON_KNOWLEDGE_PATH, 'utf-8');
            const list = JSON.parse(dataStr);
            const idx = list.findIndex(item => item.bugId === bugId);
            if (idx > -1) {
            ...
            logger.error(`Fallback database update failed: ${e.message}`);
            throw e;
        }
    },
    ...
    updateBug: async (bugId, updates) => {
        if (!isFallback) {
            try {
                return await BugModel.findOneAndUpdate({ bugId }, { $set: updates }, { new: true });
            } catch (e) {
                logger.error(`MongoDB update bug error: ${e.message}. Using fallback updating.`);
            }
        }
        ...
        try {
            ...
            logger.error(`Fallback database update bug failed: ${e.message}`);
            throw e;
        }
    },
    ...
    clearBugs: async () => {
        if (!isFallback) {
            try {
                ...
                logger.error(`MongoDB clear error: ${e.message}`);
            }
        }
        try {
            fs.writeFileSync(JSON_DB_PATH, JSON.stringify([], null, 2), 'utf-8');
        } catch (e) {
            logger.error(`Fallback database clear failed: ${e.message}`);
        }
    },
    ...
    clearKnowledge: async () => {
        if (!isFallback) {
            try {
                ...
                logger.error(`MongoDB clear knowledge error: ${e.message}`);
            }
        }
        try {
            fs.writeFileSync(JSON_KNOWLEDGE_PATH, JSON.stringify([], null, 2), 'utf-8');
        } catch (e) {
            logger.error(`Fallback database clear knowledge failed: ${e.message}`);
        }
    },
